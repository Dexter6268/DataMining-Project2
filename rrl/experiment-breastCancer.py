...
def get_data_loader(dataset, world_size, rank, batch_size, k=0, pin_memory=False, save_best=True):
    ...
    X_df, y_df, f_df, label_pos = read_csv(data_path, info_path, shuffle=False)  # 取消 shuffle，保证固定顺序
    ...
    # 固定划分前78条为训练集，后19条为测试集
    train_index = np.arange(0, 78)
    test_index = np.arange(78, 97)
    ...
    # if save_best:  # 如果需要保存最优模型，则只使用训练子集
    #     train_set = train_sub
