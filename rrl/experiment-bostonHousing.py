...
def get_data_loader(dataset, world_size, rank, batch_size, k=0, pin_memory=False, save_best=True):
    ...
    # Normalized input features and target
...
def train_model(gpu, args):
    ...
    rrl = RRL(dim_list=[(discrete_flen, continuous_flen)] + list(map(int, args.structure.split('@'))) + [1],  # Output size 1 for regression
              device_id=device_id,
...
def test_model(args):
    ...
    dataset = args.data_set
    db_enc, _, _, test_loader = get_data_loader(dataset, 4, 0, args.batch_size, args.ith_kfold, save_best=False)
    
    # Test the model
    mse, rmse = rrl.test(test_loader=test_loader, set_name='Test')

    # Print rules (optional, for interpretability)
    if args.print_rule:
        with open(args.rrl_file, 'w') as rrl_file:
            rule2weights = rrl.rule_print(db_enc.X_fname, db_enc.y_fname, None, file=rrl_file, mean=db_enc.mean, std=db_enc.std)
    else:
        rule2weights = rrl.rule_print(db_enc.X_fname, db_enc.y_fname, None, mean=db_enc.mean, std=db_enc.std, display=False)
